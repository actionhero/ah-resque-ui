...
function LocksPage ({ client }) {
  ...
  async function loadData () {
    const response = await client.action({}, '/api/resque/locks', 'GET')
    const locks = []
  ...
  async function delLock (lock) {
    if (confirm('Are you sure?')) {
      await client.action({ lock: lock }, '/api/resque/delLock', 'POST')
      await loadData()
    }
  }
                ...
                data.locks.map((l) => {
                  ...
                  return (
                    <tr key={`${index}-${l.at.getTime()}`}>
                      ...
                      <td>{l.lock}</td>
                      <td>{l.at.toString()}</td>
                      <td><button onClick={delLock.bind(null, l.lock)} className='btn btn-xs btn-warning'>Delete</button></td>
                    </tr>
                  )
                })
              }
